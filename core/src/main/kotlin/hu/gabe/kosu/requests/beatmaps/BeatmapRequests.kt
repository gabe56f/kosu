...
class BeatmapRequests(kosu: Kosu) : BaseRequestV2(kosu) {
    ...
    @RequiresOauth
    operator fun get(ids: List<Int>): List<Beatmap> = client.get(url, emptyMap(), ids.map {
        "ids[]" to it.toString()
    }).decodeFromJson<Beatmaps>(kosu).beatmaps

    @RequiresOauth
    operator fun get(id: Long): Beatmap = client.get("$url/$id", emptyMap(), emptyList()).decodeFromJson(kosu)

    @RequiresOauth
    fun getBeatmapAttributes(beatmapId: Long): BeatmapDifficultyAttributes =
        client.post("$url/$beatmapId/attributes", emptyMap()).decodeFromJson<BeatmapAttributes>(kosu).attributes
