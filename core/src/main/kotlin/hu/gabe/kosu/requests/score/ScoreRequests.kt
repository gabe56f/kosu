...
class ScoreRequests(kosu: Kosu) : BaseRequestV2(kosu) {
    ...
    @RequiresOauth
    fun getUserScoreForBeatmap(beatmapId: Long, userId: Long): BeatmapUserScore = client.get(
    ...
    @RequiresOauth
    fun getUserBeatmapScores(beatmapId: Long, userId: Long): Scores = client.get(
    ...
    @RequiresOauth
    fun getBeatmapScores(beatmapId: Long): BeatmapScores = client.get(
    ...
    @RequiresOauth
    operator fun get(scoreId: Long): Score = client.get("$endpoint/scores/$scoreId").decodeFromJson(kosu)
