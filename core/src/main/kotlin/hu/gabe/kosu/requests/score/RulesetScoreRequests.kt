...
class RulesetScoreRequests(kosu: Kosu, private val ruleset: Ruleset) : BaseRequestV2(kosu) {
    ...
    operator fun get(scoreId: Long): Score =
        client.get("$endpoint/scores/${ruleset.stringLiteral}/$scoreId").decodeFromJson(kosu)

    @RequiresOauth
    @JvmName("get")
    operator fun invoke(cursorString: String? = null): ScoreRequests.Scores = client.get(
    ...
    @RequiresOauth
    fun all(): List<Score> =
        client.get("$endpoint/scores/${ruleset.stringLiteral}").decodeFromJson<ScoreRequests.Scores>(kosu).scores

    @RequiresOauth
    fun getTopScores(beatmapId: Long, mods: List<Mod> = emptyList(), stableOnly: Boolean = false): BeatmapScores =
        client.get(
            "$endpoint/beatmaps/$beatmapId/scores", emptyMap(), listOf(
