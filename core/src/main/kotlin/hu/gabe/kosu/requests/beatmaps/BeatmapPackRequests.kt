...
class BeatmapPackRequests(kosu: Kosu) : BaseRequestV2(kosu) {
    ...
    @RequiresOauth
    operator fun get(pack: String): BeatmapPack = client.get(url, mapOf("pack" to pack)).decodeFromJson(kosu)

    @RequiresOauth
    fun all(): List<BeatmapPack> = client.get(url, mapOf("pack" to null)).decodeFromJson<BeatmapPacks>(kosu).packs
