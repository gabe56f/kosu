...
class UserRequests(kosu: Kosu) : BaseRequestV2(kosu) {
    ...
    operator fun invoke(userId: Long, mode: Ruleset? = null): User {
        canExecute()
        return client.get(url, mapOf("user" to userId.toString(), "mode" to mode?.stringLiteral))
            .decodeFromJson(kosu)
    }

    @RequiresOauth(Scope.PUBLIC, Scope.IDENTIFY)
    fun me(mode: Ruleset? = null): User {
        canExecute(Scope.PUBLIC, Scope.IDENTIFY)
        return client.get("$endpoint/me/{mode?}", mapOf("mode" to mode?.stringLiteral)).decodeFromJson(kosu)
    }

    @RequiresOauth
    fun getRecentUserActivity(userId: Long): List<Event> {
        canExecute()
        return client.get("$endpoint/users/$userId/recent_activity", emptyMap(), emptyList()).decodeFromJson(kosu)
    }
