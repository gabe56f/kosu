...
internal fun setKosu(obj: Any, kosu: Kosu): Any {
    if (obj is KosuChild) {
        ...
        obj::class.declaredMemberProperties.forEach { member ->
            if (member.getter.hasAnnotation<Ignore>()) return@forEach
            val value = member.getter.call(obj) ?: return@forEach
            if (value is KosuChild)
                setKosu(value, kosu)
            else if (value is List<*>)
                value.forEach { it?.let { setKosu(it, kosu) } }
        }
    } else if (obj is List<*>)
